...
class ResolutionBase(BaseModel):
    date: datetime = Field(examples=["2024-12-25T12:00:00Z"], description="The date when the resolution was determined")
    value: str = Field(
        examples=["true"], description="The result value. For binary questions, use 'true' or 'false'", max_length=255
    )
    description: str = Field(
        examples=["See link: http://127.0.0.1"], description="The description justifiying the resolution result"
    )
...
class ResolutionCreate(ResolutionBase):
    question_id: uuid.UUID = Field(
        description="The ID of the question being resolved", examples=["123e4567-e89b-12d3-a456-426614174000"]
    )
